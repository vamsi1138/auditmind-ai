...
export default function Analyze() {
  ...
  const activeContractCode = s.lastResult?.contractCode || s.code;
  const evidence = useMemo(() => buildEvidenceMap(activeContractCode, risks), [activeContractCode, risks]);
  ...
  const analyze = async () => {
    ...
    try {
      ...
      s.addHistoryEntry({ code: result?.contractCode || source, uploadedFiles: s.uploadedFiles, result, selectedFeature: s.selectedFeature, inputMode: s.inputMode, analysisMs: elapsed, riskScore: result?.riskScore, severity: result?.severity });
